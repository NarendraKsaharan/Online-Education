                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                        @foreach($courses as $id => $entry)
                            <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $courseVideo->course->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('topic') ? 'is-invalid' : '' }}" name="topic_id" id="topic_id" required>
                        @foreach($topics as $id => $entry)
                            <option value="{{ $id }}" {{ (old('topic_id') ? old('topic_id') : $courseVideo->topic->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('topic'))
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('video_title') ? 'is-invalid' : '' }}" type="text" name="video_title" id="video_title" value="{{ old('video_title', $courseVideo->video_title) }}" required>
              ...
              <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('video_description') ? 'is-invalid' : '' }}" name="video_description" id="video_description">{!! old('video_description', $courseVideo->video_description) !!}</textarea>
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('video_source') ? 'is-invalid' : '' }}" name="video_source" id="video_source">{{ old('video_source', $courseVideo->video_source) }}</textarea>
