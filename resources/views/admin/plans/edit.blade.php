                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('course') ? 'is-invalid' : '' }}" name="course_id" id="course_id" required>
                            @foreach($courses as $id => $entry)
                                <option value="{{ $id }}" {{ (old('course_id') ? old('course_id') : $plan->course->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                            @endforeach
                        </select>
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input class="form-control {{ $errors->has('special_price') ? 'is-invalid' : '' }}" type="number" name="special_price" id="special_price" value="{{ old('special_price', $plan->special_price) }}" step="0.01">
