                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.sliders.update", [$slider->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $slider->description) !!}</textarea>
                        ...
                        <div class="form-group">
                            <label class="required" for="slug">{{ trans('cruds.slider.fields.slug') }}</label>
                            <input class="form-control" type="text" name="slug" id="slug" value="{{ old('slug', $slider->slug) }}" required>
                            @if($errors->has('slug'))
                                <div class="invalid-feedback">
                                    {{ $errors->first('slug') }}
                                </div>
                            @endif
                            <span class="help-block">{{ trans('cruds.slider.fields.slug_helper') }}</span>
                        </div>
                        <div class="form-group">
          ...
          return loader.file
            .then(function (file) {
              return new Promise(function(resolve, reject) {
                ...
                xhr.open('POST', '{{ route('frontend.sliders.storeCKEditorImages') }}', true);
...
Dropzone.options.sliderImageDropzone = {
    ...
    success: function (file, response) {
      ...
      uploadedSliderImageMap[file.name] = response.name
    },
    removedfile: function (file) {
      ...
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedSliderImageMap[file.name]
      }
      $('form').find('input[name="slider_image[]"][value="' + name + '"]').remove()
    },
    init: function () {
@if(isset($slider) && $slider->slider_image)
      var files = {!! json_encode($slider->slider_image) !!}
          for (var i in files) {
          var file = files[i]
