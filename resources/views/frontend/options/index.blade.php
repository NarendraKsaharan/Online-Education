        ...
        <div class="col-md-12">
            @can('option_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.options.create') }}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="search" strict="true">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach(App\Models\Option::OPTION_SEQUENCE_SELECT as $key => $item)
                            ...
                            <tbody>
                                @foreach($options as $key => $option)
                                    <tr data-entry-id="{{ $option->id }}">
                                        ...
                                        <td>
                                            @can('option_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.options.show', $option->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('option_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.options.edit', $option->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('option_delete')
                                                <form action="{{ route('frontend.options.destroy', $option->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('option_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.options.massDestroy') }}",
