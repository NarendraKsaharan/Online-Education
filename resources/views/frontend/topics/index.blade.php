        ...
        <div class="col-md-12">
            @can('topic_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.topics.create') }}">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <select class="search" strict="true">
                                            <option value>{{ trans('global.all') }}</option>
                                            @foreach(App\Models\Topic::STATUS_SELECT as $key => $item)
                            ...
                            <tbody>
                                @foreach($topics as $key => $topic)
                                    <tr data-entry-id="{{ $topic->id }}">
                                        ...
                                        <td>
                                            @can('topic_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.topics.show', $topic->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('topic_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.topics.edit', $topic->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('topic_delete')
                                                <form action="{{ route('frontend.topics.destroy', $topic->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('topic_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.topics.massDestroy') }}",
